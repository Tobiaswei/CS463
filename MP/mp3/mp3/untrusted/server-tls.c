...
#define DEFAULT_PORT 10020 //TODO: change port number
...
int server_connect(sgx_enclave_id_t id)
{
    ...
    WOLFSSL_CTX* ctx;
    WOLFSSL*     ssl; //TODO: you need one WOFLSSL object for each connection
...
/* Fill in the server address */
    ...
    for(;;){
        ...
        if(select(fdmax+1,&read_fds,NULL,NULL)==-1){
            perror("select");
            exit(4);
        }
        for(int i=0;i<=fdmax;i++){
            ...
            if(FD_ISSET((i,&read_fds))){
                if(i==sockfd){
                    ...
                    /* Attach wolfSSL to the socket TODO: you need one ssl per connection */
                  ...
                  //handle data from client
                   ...
                   if(sgxStatus != SGX_SUCCESS || ret == -1){
                       ...
                       /* Read the client data into our buff array TODO: enc_wolfSSL_read is also being used by client */
                       ...
                       /* Reply back to the client
                          TODO: enc_wolfSSL_write is also being used by client */
    ...
    /* TODO: your server MUST accept at least three clients Hint: use FD_SET and select() to handle multiple connections */
